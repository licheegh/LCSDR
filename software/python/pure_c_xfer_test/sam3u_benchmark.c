...
static void measure(void)
{
	...
	printf("\n %lu transfers (total %lu bytes) in %u DATA_INmiliseconds => %.2f MB/sec\n",
		num_xfer, num_bytes, diff_msec, ((float)num_bytes/1000)/diff_msec);
}
...
int main(int argc, char **argv)
{
	...
	devh = libusb_open_device_with_vid_pid(NULL, 0x04b4, 0x00f1);
	if (!devh) {
